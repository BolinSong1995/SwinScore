...
Step 0 — Compute bounding boxes (one-time setup). Make sure you have a file called "data_table.csv" within path ".\p16_positive_opc_ct\radiology\". 
...
Step 1 — Compute bounding boxes (one-time setup), IMPORTANT: you have to have a file called "data_table.csv" (clinical sheet in the above step) within path ".\p16_positive_opc_ct\radiology\" to be able to run this step, it will output the "data_table_test.csv" with new columns of: X_min_tumor, Y_min_tumor, X_max_tumor, Y_max_tumor, Z_min_tumor, Z_max_tumor, X_min_lymph, Y_min_lymph, X_max_lymph, Y_max_lymph, Z_min_lymph, Z_max_lymph
command: python preprocessing.py

Step 2 — Inference, this step will read the "data_table_test.csv", the CT images within each patient subfolder, and the checkpoint file to make inference.
