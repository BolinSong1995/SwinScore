...
def one_epoch(args, split, model, optim, loader, criterion):
    ...
    for i, (mod1, mod2, grade, time, event, ID) in enumerate(loader):
        ...
        if args.task == "multitask":
            ...
            pred_grade = pred_grade.view(-1)
            pred_hazard = pred_hazard.view(-1)

        elif args.task == "classification":
            pred_grade = pred.view(-1)
            pred_hazard = torch.empty(0, device=device)

        elif args.task == "survival":
            pred_grade = torch.empty(0, device=device)
            pred_hazard = pred.view(-1)

        else:
            raise NotImplementedError(f'task method {args.task} is not implemented')

        grade = grade.float().view(-1)
        time = time.view(-1)
        event = event.view(-1)

        loss = criterion(args.task, pred_grade, pred_hazard, grade, time, event)
    ...
    if args.task == "classification" :
        all_preds_grade = torch.concat(all_preds_grade)/10-1
...
def test(args, device):
    ...
    print(
        ...
        # Unpack the tuple
    all_preds_grade, _, all_grade, all_time, all_event, all_ID = test_preds

    print("SwinScore (first 30):")
    print(all_preds_grade[:30] if hasattr(all_preds_grade, '__getitem__') else all_preds_grade)
    print()

    print("time in month (first 30):")
    print(all_time[:30] if hasattr(all_time, '__getitem__') else all_time)
    print()

    print("event (first 30):")
    print(all_event[:30] if hasattr(all_event, '__getitem__') else all_event)
    print()

    print("all_ID (first 30):")
    print(all_ID[:30] if hasattr(all_ID, '__getitem__') else all_ID)
